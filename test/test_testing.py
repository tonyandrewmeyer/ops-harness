...
class TestHarness:
        ...
        charm_meta = """
            ...
            requires:
                db:
                    interface: pgsql
        """
        harness = testing.Harness(ops.CharmBase, meta=charm_meta)
        request.addfinalizer(harness.cleanup)
        rel_id = harness.add_relation('db', 'postgresql')
        ...
        metadata = """
            ...
            requires:
              db:
                interface: pgsql
            """
        harness1 = testing.Harness(ops.CharmBase, meta=metadata)
        request.addfinalizer(harness1.cleanup)
        harness2 = testing.Harness(ops.CharmBase, meta=metadata)
    ...
    def test_empty_config_raises(self):
        with pytest.raises(TypeError):
            testing.Harness(RecordingCharm, config='')
    ...
    def test_update_config_undefined_option(self, request: pytest.FixtureRequest):
        harness = testing.Harness(RecordingCharm)
        request.addfinalizer(harness.cleanup)
        harness.begin()
    ...
    def _get_dummy_charm_harness(self, request: pytest.FixtureRequest, tmp: pathlib.Path):
        ...
        harness = testing.Harness(charm_mod.MyTestingCharm)
        request.addfinalizer(harness.cleanup)
        return harness
    ...
    def test_unit_status(self, request: pytest.FixtureRequest):
        harness = testing.Harness(ops.CharmBase, meta='name: test-app')
        request.addfinalizer(harness.cleanup)
        harness.set_leader(True)
        harness.begin()
        # default status
    ...
    def test_evaluate_status(self):
        ...
        harness = testing.Harness(TestCharm)
        harness.set_leader(True)
        harness.begin()
...
class TestNetwork:
    ...
    def test_add_network_defaults(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_add_network_all_args(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_add_network_specific_endpoint(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_add_network_specific_relation(self, harness: testing.Harness[ops.CharmBase]):
        harness.add_network('10.0.0.1')
        harness.add_network('10.0.2.1', endpoint='db')
        relation_id = harness.add_relation('db', 'postgresql')
    ...
    def test_add_network_endpoint_fallback(self, harness: testing.Harness[ops.CharmBase]):
        relation_id = harness.add_relation('db', 'postgresql')
    ...
    def test_add_network_default_fallback(self, harness: testing.Harness[ops.CharmBase]):
        harness.add_network('10.0.0.10')
    ...
    def test_add_network_ipv6(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_network_get_relation_not_found(self, harness: testing.Harness[ops.CharmBase]):
        with pytest.raises(ops.RelationNotFoundError):
            binding = harness.model.get_binding('db')
            assert binding is not None
            binding.network
    ...
    def test_add_network_endpoint_not_in_meta(self, harness: testing.Harness[ops.CharmBase]):
        with pytest.raises(ops.ModelError):
            harness.add_network('35.0.0.1', endpoint='xyz')

    def test_add_network_relation_id_set_endpoint_not_set(
        self, harness: testing.Harness[ops.CharmBase]
    ):
        relation_id = harness.add_relation('db', 'postgresql')
        with pytest.raises(TypeError):
            harness.add_network('35.0.0.1', relation_id=relation_id)

    def test_add_network_relation_id_incorrect(self, harness: testing.Harness[ops.CharmBase]):
        relation_id = harness.add_relation('db', 'postgresql')
        with pytest.raises(ops.ModelError):
...
class TestFilesystem:
    ...
    @pytest.fixture
    def container_fs_root(self, harness: testing.Harness[ops.CharmBase]):
        return harness.get_filesystem_root('test-container')

    @pytest.fixture
    def container(self, harness: testing.Harness[ops.CharmBase]):
            ...
            meta = """
                ...
                storage:
                    test-storage:
                        type: filesystem
                """
        harness = testing.Harness(MyCharm, meta=meta)
        request.addfinalizer(harness.cleanup)
        return harness
...
class TestSecrets:
    ...
    def test_get_secret_and_refresh(self, request: pytest.FixtureRequest):
        harness = testing.Harness(ops.CharmBase, meta='name: webapp')
        request.addfinalizer(harness.cleanup)
        harness.begin()
        harness.set_leader(True)
    ...
    def test_trigger_secret_rotation(self, request: pytest.FixtureRequest):
        harness = testing.Harness(EventRecorder, meta='name: database')
        request.addfinalizer(harness.cleanup)
    ...
    def test_add_user_secret(self, request: pytest.FixtureRequest):
        harness = testing.Harness(ops.CharmBase, meta=yaml.safe_dump({'name': 'webapp'}))
        request.addfinalizer(harness.cleanup)
        harness.begin()
...
class TestHandleExec:
    ...
    def test_exec_timeout(
        ...
        def handler(_: testing.ExecArgs):
            raise TimeoutError
...
class TestActions:
    ...
    def test_invalid_action(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_run_action(self, harness: testing.Harness[ops.CharmBase]):
        out = harness.run_action('simple')
        assert out.logs == []
        assert out.results == {}
        assert harness.charm.simple_was_called  # type: ignore

    def test_fail_action_no_message(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_required_param(self, harness: testing.Harness[ops.CharmBase]):
        with pytest.raises(RuntimeError):
    ...
    def test_additional_params(self, harness: testing.Harness[ops.CharmBase]):
    ...
    def test_logs_and_results(self, harness: testing.Harness[ops.CharmBase]):
