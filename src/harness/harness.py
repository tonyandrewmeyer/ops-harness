...
class Harness(Generic[CharmType]):
    ...
    def __init__(
        ...
        *,
        meta: YAMLStringOrFile | None = None,
        actions: YAMLStringOrFile | None = None,
        config: YAMLStringOrFile | None = None,
    ):
    ...
    def _create_meta(
        self,
        charm_metadata_yaml: YAMLStringOrFile | None,
        action_metadata_yaml: YAMLStringOrFile | None,
        config_metadata_yaml: YAMLStringOrFile | None,
    ) -> CharmMeta:
        ...
        charm_metadata: dict[str, Any] | None = None
        charmcraft_metadata: dict[str, Any] | None = None
        if charm_dir:
    ...
    def _get_config(self, charm_config_yaml: YAMLStringOrFile | None):
    ...
    def add_oci_resource(
        self, resource_name: str, contents: Mapping[str, str] | None = None
    ) -> None:
    ...
    def add_relation(
        ...
        remote_app: str,
        *,
        app_data: Mapping[str, str] | None = None,
        unit_data: Mapping[str, str] | None = None,
    ) -> int:
    ...
    def trigger_secret_rotation(self, secret_id: str, *, label: str | None = None):
    ...
    def trigger_secret_removal(self, secret_id: str, revision: int, *, label: str | None = None):
    ...
    def trigger_secret_expiration(
        self, secret_id: str, revision: int, *, label: str | None = None
    ):
    ...
    def get_filesystem_root(self, container: str | Container) -> pathlib.Path:
    ...
    def handle_exec(
        ...
        *,
        ...
        result: int | str | bytes | ExecResult | None = None,
    ):
...
class _TestingConfig(typing.Dict[str, typing.Union[str, int, float, bool]]):
    ...
    @staticmethod
    def _load_defaults(charm_config: _RawConfig) -> dict[str, str | int | float | bool]:
    ...
    def _config_set(self, key: str, value: str | int | float | bool):
...
class _TestingModelBackend:
    ...
    def __init__(
        ...
        self._resources_map: dict[str, tuple[str, str | bytes]] = {}
        # fixme: understand how this is used and adjust the type
        self._pod_spec: tuple[model.K8sSpec, Any] | None = None
        ...
        self._running_action: _RunningAction | None = None
    ...
    def network_get(self, endpoint_name: str, relation_id: int | None = None) -> _NetworkDict:
    ...
    def _get_secret(self, id: str) -> _Secret | None:
    ...
    def _ensure_secret_id_or_label(self, id: str | None, label: str | None):
        secret = None
        if id is not None:
            secret = self._get_secret(id)
    ...
    def _relation_id_to(self, remote_app: str) -> int | None:
    ...
    def secret_info_get(
        self, *, id: str | None = None, label: str | None = None
    ) -> model.SecretInfo:
        secret = self._ensure_secret_id_or_label(id, label)
    ...
    def secret_set(
        ...
        *,
        ...
        rotate: model.SecretRotate | None = None,
    ) -> None:
    ...
    def secret_grant(self, id: str, relation_id: int, *, unit: str | None = None) -> None:
        secret = self._ensure_secret(id)
        if not self._has_secret_owner_permission(secret):
            raise model.SecretNotFoundError(
                f'You must own secret {secret.id!r} to perform this operation'
    ...
    def secret_revoke(self, id: str, relation_id: int, *, unit: str | None = None) -> None:
        secret = self._ensure_secret(id)
        if not self._has_secret_owner_permission(secret):
            raise RuntimeError(f'You must own secret {secret.id!r} to perform this operation')
    ...
    def secret_remove(self, id: str, *, revision: int | None = None) -> None:
        secret = self._ensure_secret(id)
        if not self._has_secret_owner_permission(secret):
            raise RuntimeError(f'You must own secret {secret.id!r} to perform this operation')
    ...
    def _check_protocol_and_port(self, protocol: str, port: int | None):
...
class _TestingPebbleClient:
    ...
    def _find_exec_handler(self, command: list[str]) -> ExecHandler | None:
    ...
    def _transform_exec_handler_output(
        self, data: str | bytes, encoding: str | None
    ...
    def get_checks(
        self, level: pebble.CheckLevel | None = None, names: Iterable[str] | None = None
    ...
    def replace_identities(
        self, identities: dict[str, pebble.IdentityDict | pebble.Identity | None]
    ) -> None:
        raise NotImplementedError
